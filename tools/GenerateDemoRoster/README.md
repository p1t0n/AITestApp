...
`api/Infrastructure/Persistence/SeedData/demo-roster.json`: 500 synthetic experts across
...
2. **Narrative prose** — every experience gets a summary + 2–5 achievement bullets from
   ...
   parameterized so no summary text repeats more than 3 times. ~13% of experts are written
...
3. **Optional LLM polish** — when `GEMINI_API_KEY` is set, the tool rewrites the fragment
   ...
   `openai/gpt-4o-mini`) in batches of 4 experts. The pass is best-effort: any batch that
...
end to end, but the full 500-expert enrichment run (~25 minutes of batched calls) did not
