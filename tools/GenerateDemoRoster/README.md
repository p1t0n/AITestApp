...
3. **Optional LLM polish** — when `GITHUB_TOKEN` is set, the tool rewrites the fragment
   prose via the GitHub Models endpoint (`https://models.github.ai/inference`, model
...
```bash
# needs a GitHub token with access to GitHub Models for the LLM polish pass
...
Never commit the token anywhere. Without `GITHUB_TOKEN` the tool automatically runs offline.
...
bit-for-bit reproducible). The GitHub Models polish pass was implemented and smoke-verified
...
passes the same variety guards. Rerun with `GITHUB_TOKEN` set to produce an LLM-polished
