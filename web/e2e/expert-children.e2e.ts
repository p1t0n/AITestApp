 ...
 */
test.describe("expert child editing", () => {
  ...
  test("an experience with bullets and a skill is written, edited, and reaches the CV", async ({
    ...
    const detailUrl = await createExpert(page, "Barbara", "Liskov");
  ...
  test("a language and a qualification round trip to the CV, and delete removes them", async ({
    ...
    const detailUrl = await createExpert(page, "Grace", "Murray");
  ...
  test("availability and a skill are added, then edited through the PUTs that had no caller", async ({
    ...
    await createExpert(page, "Katherine", "Johnson");
    ...
    // Locked, because `ExpertSkillService.UpdateAsync` assigns the level and the years only.
  ...
  test("a validation failure from the API is shown in the child dialog, not swallowed", async ({
    ...
    await createExpert(page, "Alan", "Turing");
