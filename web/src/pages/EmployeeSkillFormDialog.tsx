...
import type { SaveEmployeeSkill, SkillDto, SkillLevel } from "../types";
...
interface Props {
  open: boolean;
  title: string;
  initial?: Partial<SaveEmployeeSkill>;
  /**
  ...
  onClose: () => void;
  onSave: (dto: SaveEmployeeSkill) => Promise<unknown>;
}

const empty: SaveEmployeeSkill = { skillId: "", level: "Intermediate", yearsExperience: 1 };

/**
 * A skill on an employee is a link to a catalog row plus a level and a year count. The catalog row
 ...
 */
export default function EmployeeSkillFormDialog({
  open,
  title,
  initial,
  lockedSkillName,
  ...
  const [form, setForm] = useState<SaveEmployeeSkill>({ ...empty, ...initial });
