...
interface Props {
  open: boolean;
  title: string;
  initial?: Partial<SaveEmployee>;
  onClose: () => void;
  onSave: (dto: SaveEmployee) => Promise<unknown>;
}

const empty: SaveEmployee = {
  firstName: "",
  lastName: "",
  title: "",
  email: "",
...
export default function EmployeeFormDialog({ open, title, initial, onClose, onSave }: Props) {
  const [form, setForm] = useState<SaveEmployee>({ ...empty, ...initial });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const field =
    (key: keyof SaveEmployee) =>
