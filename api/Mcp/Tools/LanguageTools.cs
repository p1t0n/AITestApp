...
public class LanguageTools
{
    [McpServerTool(Name = "language_add", ReadOnly = false, Destructive = false),
     Description(
         ...
         "programming language — that is a catalog skill: expert_skill_add (with skill_create " +
         "first if the catalog lacks it); do NOT use it to change an existing entry's level — " +
         "language_update by the spoken-language id from expert_get. Input: expertId + dto " +
         "{language, level}; e.g. {\"expertId\": \"7b2e8d3a-1111-2222-3333-444455556666\", " +
    ...
    [McpServerTool(Name = "language_update", ReadOnly = false, Destructive = false),
     Description(
         ...
         "SPOKEN-LANGUAGE row id (from expert_get), not the expert id. Use it for 'her German " +
         "is Fluent, not Professional'. Do NOT use it to add a language the person does not have " +
         "yet — language_add with the expert id. Input: id + dto {language, level} (full " +
    ...
    [McpServerTool(Name = "language_delete", ReadOnly = false, Destructive = true, Idempotent = true),
     Description(
         ...
         "id (from expert_get). Do NOT use it to correct a level — language_update; the expert " +
