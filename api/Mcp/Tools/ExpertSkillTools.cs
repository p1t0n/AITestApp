...
[McpServerToolType]
public class ExpertSkillTools
{
    [McpServerTool(Name = "expert_skill_add", ReadOnly = false, Destructive = false),
     Description(
         ...
         "PostgreSQL', 'add Kubernetes at Advanced level to this expert'. It does NOT create the " +
         ...
         "an existing level or years — expert_skill_update by the expert-skill id from " +
         "expert_get. Input: expertId + dto {skillId, level, yearsExperience}; e.g. " +
         "{\"expertId\": \"7b2e8d3a-1111-2222-3333-444455556666\", \"dto\": {\"skillId\": " +
    ...
    [McpServerTool(Name = "expert_skill_update", ReadOnly = false, Destructive = false),
     Description(
         ...
         "points at — addressed by the EXPERT-SKILL id (the row's own id from expert_get or " +
         "cv_get), not by the expert id and not by the catalog skill id. Use it for 'raise her " +
         "PostgreSQL to Expert', 'correct the years on that skill'. Do NOT use it to give someone " +
         "a skill they do not have yet — expert_skill_add; do NOT use it to rename the skill " +
    ...
    [McpServerTool(Name = "expert_skill_delete", ReadOnly = false, Destructive = true, Idempotent = true),
     Description(
         "DESTRUCTIVE: remove one skill from ONE PERSON, addressed by the expert-skill row id " +
         "(from expert_get). The catalog skill itself is untouched and every other expert keeps " +
         "it. Do NOT use it to remove a skill from the catalog for everyone — skill_delete does " +
         "that; do NOT use it to lower a level — expert_skill_update. Input: id; e.g. {\"id\": " +
