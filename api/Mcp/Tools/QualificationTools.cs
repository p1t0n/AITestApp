...
public class QualificationTools
{
    [McpServerTool(Name = "qualification_add", ReadOnly = false, Destructive = false),
     Description(
         ...
         "employee_skill_add records proficiency, this records the credential; do NOT use it for a " +
         "job — experience_add; do NOT use it to amend a qualification the person already has — " +
         "qualification_update by its id. Input: employeeId + dto {type, name, institution?, field?, " +
         "startDate?, endDate?, issuer?, credentialId?, issueDate?, expiryDate?} with dates as " +
         "yyyy-MM-dd; e.g. {\"employeeId\": \"7b2e8d3a-1111-2222-3333-444455556666\", " +
    ...
    public static Task<object> Add(
        ...
        [Description("Employee id (GUID) holding the qualification.")] Guid employeeId,
    ...
    [McpServerTool(Name = "qualification_update", ReadOnly = false, Destructive = false),
     Description(
         "CHANGE one existing qualification by QUALIFICATION id (from employee_get or cv_get) — its " +
         ...
         "survive. Do NOT " +
         "use it to add a second qualification — qualification_add with the employee id. Input: id " +
    ...
    public static Task<object> Update(
        IQualificationService qualifications,
        [Description("Qualification id (GUID) from employee_get / cv_get — not the employee id.")]
        Guid id,
    ...
    [McpServerTool(Name = "qualification_delete", ReadOnly = false, Destructive = true, Idempotent = true),
     Description(
         "DESTRUCTIVE: delete one degree or certification by qualification id (from employee_get). " +
