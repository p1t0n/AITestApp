...
public class CatalogTools
{
    ...
    [McpServerTool(Name = "category_list", ReadOnly = true, Destructive = false),
     Description(
         ...
         "catalog skill; do NOT use it for one person's skills — expert_get returns those with " +
    ...
    [McpServerTool(Name = "category_tree", ReadOnly = true, Destructive = false),
     Description(
         ...
         "category — skill_list; do NOT use it for one person's skills — expert_get. Input: " +
         "none; e.g. {}. Returns catalog structure only: no expert data, no proficiency levels " +
         "or years."),
     Authorize(Policy = McpScopes.Read)]
    ...
    [McpServerTool(Name = "category_delete", ReadOnly = false, Destructive = true, Idempotent = true),
     Description(
         ...
         "the catalog vocabulary is shared by every expert. Do NOT use it to remove one skill — " +
         "skill_delete (catalog) or expert_skill_delete (one person); do NOT use it to rename — " +
    ...
    [McpServerTool(Name = "skill_list", ReadOnly = true, Destructive = false),
     Description(
         ...
         "to resolve ONE skill id (before expert_skill_add, or to get a skillId to filter a " +
         ...
         "expert_get; do NOT use it to add anything — skill_create adds a NEW catalog skill, " +
         "expert_skill_add attaches an EXISTING one to an expert. Input, all optional: " +
    ...
    [McpServerTool(Name = "skill_create", ReadOnly = false, Destructive = false),
     Description(
         "Add a NEW SKILL TO THE CATALOG — the shared vocabulary every expert is tagged from — " +
         ...
         "start tagging people'. It touches NO expert. Do NOT use it to say a PERSON has a skill " +
         "— expert_skill_add attaches an existing catalog skill to someone, and if the skill " +
    ...
    [McpServerTool(Name = "skill_update", ReadOnly = false, Destructive = false),
     Description(
         ...
         "applies to the shared vocabulary, so it shows up for every expert tagged with that " +
         "skill. Use it to fix a catalog typo or re-file a skill. Do NOT use it to change one " +
         "person's level or years — expert_skill_update edits that person's row; do NOT use it " +
         "to add a skill — skill_create (catalog) or expert_skill_add (a person). Input: id + " +
    ...
    [McpServerTool(Name = "skill_delete", ReadOnly = false, Destructive = true, Idempotent = true),
     Description(
         ...
         "skill off ONE person — expert_skill_delete removes their row and leaves the catalog " +
