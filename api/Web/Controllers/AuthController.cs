    ...
    TimeProvider clock) : ControllerBase
{
    ...
    public async Task<ActionResult<AuthSessionResponse>> SignupComplete(SignupCompleteRequest request, CancellationToken ct)
    {
        ...
        return Ok(new AuthSessionResponse(token, expiresAt, user.Id, user.Email, user.Role));
    }
...
public sealed record AuthSessionResponse(
    string Token, DateTimeOffset ExpiresAt, Guid UserId, string Email, UserRole Role);
