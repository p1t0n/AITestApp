...
| Multi-tool sequencing | Prompt-encoded numbered procedures (skill_list → expert_create_draft → children; "at most 2 retries, then skip"; hard-stop rule) in [ResumeIngestionAgent.cs](../api/Agents/Agents/ResumeIngestionAgent.cs) — matches Anthropic's guidance in spirit, enforced only by instructions. |
