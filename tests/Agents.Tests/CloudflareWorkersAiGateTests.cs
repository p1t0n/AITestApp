...
internal static class RosterScanGateChunk
{
    ...
    private static async Task<IReadOnlyList<ExpertDigest>> BuildAsync()
    {
        ...
        // Seeded rows are staff-created, so they sit on legitimate interest — which carries no
        // Art. 22(2) route, so the scan does not enumerate them at all (P1T-185). Claiming them is
        // what a production chunk is made of, so the gate claims them here rather than measuring an
        // empty page: the point of the fixture is to price a real chunk, and an unclaimed bench is
        // not one.
        foreach (var expert in await db.Experts.AsNoTracking()
                     .Select(e => new { e.Id, Sequence = e.ProcessingRecords.Count })
                     .ToListAsync())
        {
            // Added through the set with the FK set by hand, not through the navigation: a Guid key
            ...
            db.ProcessingRecords.Add(ProcessingRecord.For(
                expert.Id, expert.Sequence + 1, ProcessingOrigin.SelfRegistered,
                TransparencyNotice.CurrentVersion, "Claim approved (gate fixture).",
